...
int main(void) {
  ...
  srv = sg_httpsrv_new(srv_req_cb, NULL);
  ASSERT(srv);
  curl = curl_easy_init();
  ASSERT(curl);
  res = sg_str_new();
  ...
  ASSERT(sg_httpsrv_tls_listen(srv, private_key, certificate,
                               TEST_HTTPSRV_TLS_CURL_PORT, false));

  ASSERT(curl_easy_setopt(curl, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_1) ==
         CURLE_OK);
  snprintf(url, sizeof(url), "https://localhost:%d",
           TEST_HTTPSRV_TLS_CURL_PORT);
