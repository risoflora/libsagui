...
static void test__httpsrv_oel(const char *fmt, ...) {
    ...
    char err[256];
    va_list ap;
    srv = sg_httpsrv_new2(dummy_httpreq_httpauth_cb, dummy_httpreq_cb, dummy_httpreq_err_cpy_cb, err);
    ASSERT(srv);
    va_start(ap, fmt);
...
static void test_httpsrv_new2(void) {
    ...
    srv = sg_httpsrv_new2(dummy_httpreq_httpauth_cb, dummy_httpreq_cb, dummy_httpreq_err_cb, &dummy);
    ASSERT(srv);
