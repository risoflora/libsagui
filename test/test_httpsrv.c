...
static bool dummy_httpreq_httpauth_cb(void *cls, struct sg_httpauth *auth,
...
static ssize_t dummy_httpupld_write_cb(void *handle, uint64_t offset,
...
static int dummy_httpupld_save_as_cb(void *handle, const char *path,
                                     bool overwritten) {
  (void) handle;
  (void) path;
  (void) overwritten;
...
static void test__httpsrv_oel(const char *fmt, ...) {
  ...
  char err[256];
  va_list ap;
  srv = sg_httpsrv_new2(dummy_httpreq_httpauth_cb, dummy_httpreq_cb,
...
static void test_httpsrv_new2(void) {
  ...
  errno = 0;
  ASSERT(!sg_httpsrv_new2(dummy_httpreq_httpauth_cb, NULL, dummy_httpreq_err_cb,
                          &dummy));
  ASSERT(errno == EINVAL);
  errno = 0;
  ASSERT(!sg_httpsrv_new2(dummy_httpreq_httpauth_cb, dummy_httpreq_cb, NULL,
                          &dummy));
  ASSERT(errno == EINVAL);
...
static void test_httpsrv_new(void) {
  ...
  sg_httpsrv_free(srv);
  errno = 0;
  srv = sg_httpsrv_new(dummy_httpreq_cb, &dummy);
  ASSERT(srv);
  ASSERT(errno == 0);
  sg_httpsrv_free(srv);
}
...
static void test_httpsrv_tls_listen(struct sg_httpsrv *srv) {
  ...
  errno = 0;
  ASSERT(!sg_httpsrv_tls_listen(NULL, private_key, certificate,
                                TEST_HTTPSRV_PORT, true));
  ASSERT(errno == EINVAL);
  errno = 0;
  ASSERT(
    !sg_httpsrv_tls_listen(srv, NULL, certificate, TEST_HTTPSRV_PORT, true));
  ASSERT(errno == EINVAL);
  errno = 0;
  ASSERT(
    !sg_httpsrv_tls_listen(srv, private_key, NULL, TEST_HTTPSRV_PORT, true));
  ASSERT(errno == EINVAL);
  dummy_srv = sg_httpsrv_new(dummy_httpreq_cb, NULL);
  dummy_srv->upld_cb = NULL;
  errno = 0;
  ...
  ASSERT(sg_httpsrv_tls_listen(srv, private_key, certificate, TEST_HTTPSRV_PORT,
                               false));
  ASSERT(MHD_get_daemon_info(srv->handle, MHD_DAEMON_INFO_FLAGS)->flags &
         MHD_USE_DUAL_STACK);
  ASSERT(MHD_get_daemon_info(srv->handle, MHD_DAEMON_INFO_FLAGS)->flags &
...
static void test__httpsrv_set_upld_cbs(struct sg_httpsrv *srv) {
  ...
  ASSERT(sg_httpsrv_set_upld_cbs(NULL, dummy_httpupld_cb, &dummy,
                                 dummy_httpupld_write_cb,
                                 dummy_httpupld_free_cb, dummy_httpupld_save_cb,
                                 dummy_httpupld_save_as_cb) == EINVAL);
  ASSERT(sg_httpsrv_set_upld_cbs(srv, NULL, &dummy, dummy_httpupld_write_cb,
                                 dummy_httpupld_free_cb, dummy_httpupld_save_cb,
                                 dummy_httpupld_save_as_cb) == EINVAL);
  ASSERT(sg_httpsrv_set_upld_cbs(srv, dummy_httpupld_cb, &dummy, NULL,
                                 dummy_httpupld_free_cb, dummy_httpupld_save_cb,
                                 dummy_httpupld_save_as_cb) == EINVAL);
  ASSERT(sg_httpsrv_set_upld_cbs(
           srv, dummy_httpupld_cb, &dummy, dummy_httpupld_write_cb,
           dummy_httpupld_free_cb, NULL, dummy_httpupld_save_as_cb) == EINVAL);
  ASSERT(sg_httpsrv_set_upld_cbs(
           srv, dummy_httpupld_cb, &dummy, dummy_httpupld_write_cb,
           dummy_httpupld_free_cb, dummy_httpupld_save_cb, NULL) == EINVAL);

  ASSERT(sg_httpsrv_set_upld_cbs(srv, dummy_httpupld_cb, &dummy,
                                 dummy_httpupld_write_cb,
                                 dummy_httpupld_free_cb, dummy_httpupld_save_cb,
                                 dummy_httpupld_save_as_cb) == 0);
