...
static int routes_iter_empty_cb(__SG_UNUSED void *cls, __SG_UNUSED struct sg_route *route) {
    return 0;
}

static int routes_iter_123_cb(__SG_UNUSED void *cls, __SG_UNUSED struct sg_route *route) {
    return 123;
}
...
static void test_routes_add2(void) {
    ...
    struct sg_route *route;
    char err[SG_ERR_SIZE];
    ASSERT(sg_routes_add2(NULL, &route, "/foo", err, sizeof(err), route_cb, "foo") == EINVAL);
    ASSERT(sg_routes_add2(&routes, NULL, "/foo", err, sizeof(err), route_cb, "foo") == EINVAL);
    ASSERT(sg_routes_add2(&routes, &route, NULL, err, sizeof(err), route_cb, "foo") == EINVAL);
    ASSERT(sg_routes_add2(&routes, &route, "/foo", NULL, sizeof(err), route_cb, "foo") == EINVAL);
    ASSERT(sg_routes_add2(&routes, &route, "/foo", err, 0, route_cb, "foo") == EINVAL);
    ASSERT(sg_routes_add2(&routes, &route, "/foo", err, sizeof(err), NULL, "foo") == EINVAL);
    ASSERT(sg_routes_add2(&routes, &route, "/foo\\K/bar", err, sizeof(err), route_cb, "foo") == EINVAL);

    ASSERT(sg_routes_add2(&routes, &route, "foo", err, sizeof(err), route_cb, "foo") == 0);
    ASSERT(sg_routes_add2(&routes, &route, "foo", err, sizeof(err), route_cb, "foo") == EALREADY);
    sg_routes_cleanup(&routes);

    ASSERT(sg_routes_add2(&routes, &route, "foo", err, sizeof(err), route_cb, "foo") == 0);
    ASSERT(route->cb == route_cb);
    ASSERT(strcmp(route->cls, "foo") == 0);
    ASSERT(sg_routes_add2(&routes, &route, "bar", err, sizeof(err), route_cb, "foo") == 0);
    ASSERT(sg_routes_add2(&routes, &route, "foobar", err, sizeof(err), route_cb, "foobar") == 0);
    route = routes;
    ASSERT(strcmp(sg_route_rawpattern(route), "^foo$") == 0);
